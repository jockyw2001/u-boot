...
#define EN_TR_MAIN               (FALSE)
#define EN_TR_SUB                (FALSE)

#define EN_SDMMC_TRFUNC          (TRUE)
#define EN_SDMMC_TRSDINFO        (FALSE)

#define D_SDMMC_DRVTYPE          (SD_DRIVER_TYPE_B)
#define D_SDMMC_CURRLMT          (SD_CURR_LMT_200)
#define D_SDMMC_BUSMODE          (SD_MODE_HIGH_SPEED|SD_MODE_UHS_SDR25|SD_MODE_UHS_SDR50|SD_MODE_UHS_SDR104|SD_MODE_UHS_DDR50)
...
static void _SDMMC_InfoInit(U8_T u8Slot, U8_T u8Config)
{

    _stSDMMCInfo[u8Slot].eHCS  = EV_SCS;   // STD Capacity

    if(u8Config & SD_CONFIG_SD30)
        _stSDMMCInfo[u8Slot].eSVOL = EV_S18; //SD 3.0 Voltage
    else
        _stSDMMCInfo[u8Slot].eSVOL = EV_S33;
...
{
    IPEmType eIP = ge_IPSlot[u8Slot];
    U32_T u32ReqOCR = SD_OCR_RANGE | _stSDMMCInfo[u8Slot].eHCS | _stSDMMCInfo[u8Slot].eSVOL;
    U16_T u16Count = 0;
...
static RspStruct* _SDMMC_Identification(U8_T u8Slot)
{
    ...
    if(_stSDMMCInfo[u8Slot].eSVOL)
    {

        //------------------------------------------------------------------------------------------------------------
        _pstRsp[eIP] = _SDMMC_CMDReq(u8Slot,  11, 0x00000000, EV_R1); //CMD11;
        //------------------------------------------------------------------------------------------------------------
        if(_pstRsp[eIP]->eErrCode)
            return _pstRsp[eIP];


        SDMMC_SetPADVdd(u8Slot, EV_BUS_1_8V);
...
void SDMMC_SwitchPAD(U8_T u8Slot)
{
    ...
    Hal_CARD_SetPADToPortPath(eIP, ePort, ePAD);
...
void SDMMC_SetPower(U8_T u8Slot, PowerEmType ePower)
{
    ...
    if(ePower == EV_POWER_OFF) // Power Off
    {
        ...
        Hal_CARD_PowerOff(eIP, ePAD, WT_POWEROFF); //For SD PAD
    ...
    else if(ePower == EV_POWER_ON)  // Power Up
    {
        ...
        Hal_CARD_PowerOn(eIP, ePAD, WT_POWERUP);
    }
...
/*----------------------------------------------------------------------------------------------------------
 *
 * Function: SDMMC_SetPADVdd
 *     @author jeremy.wang (2017/5/25)
 * Desc:
 *
 * @param u8Slot :
 * @param eBusVdd :
 ----------------------------------------------------------------------------------------------------------*/
void SDMMC_SetPADVdd(U8_T u8Slot, BusVddEmType eBusVdd)
{
    IPEmType eIP      = ge_IPSlot[u8Slot];
    PADEmType ePAD    = ge_PADSlot[u8Slot];

    if(eBusVdd ==EV_BUS_1_8V)
    {
        Hal_SDMMC_ClkCtrl(eIP, FALSE, 5);
        Hal_CARD_SetPADVdd(eIP, ePAD, EV_LOWVOL, 10);
        Hal_SDMMC_ClkCtrl(eIP, TRUE, 5);
    }
    else
    {
        Hal_CARD_SetPADVdd(eIP, ePAD, EV_NORVOL, 0);
    }
...
#if(EN_SDMMC_CDZREV)
    return !Hal_CARD_GetGPIOState((GPIOEmType)u8Slot, ePAD);
#else
    return Hal_CARD_GetGPIOState((GPIOEmType)u8Slot, ePAD);
#endif
...
U16_T SDMMC_SwitchHighBus(U8_T u8Slot)
{
    ...
    BusTimingEmType eBusTiming = EV_BUS_LOW;
    ...
    if(_stSDMMCInfo[u8Slot].eCardType==EV_SD)
    {
        ...
        {
            _stSDMMCInfo[u8Slot].u32MaxClk = CLK_DEF_SPEED;
            SDMMC_SetBusTiming(u8Slot, EV_BUS_DEF_SDR12);

            TR_S(prtstring("[NOT_SUPP] SD1.0 (DS)\r\n"));
        ...
        if( !(_stSDMMCInfo[u8Slot].u32OCR & R_OCR_S18A)) // SD 2.0, SD3.0 => SD2.0
        {
            ...
            if(u8BusMode & SD_MODE_HIGH_SPEED) // Support High Speed
            {
                ...
                //------------------------------------------------------------------------------------------------------------
                TR_S(prtstring("[SET_GRP1] SD2.0 (HS) = "));

                if(_pstRsp[eIP]->eErrCode)
                {
                    TR_S(prtstring("Rsp Fail !\r\n"));
                    return (U16_T)_pstRsp[eIP]->eErrCode;
                }
                else if( (gu8RspBuf[u8Slot][16] & 0xF) != _stSDMMCInfo[u8Slot].u8AccessMode )
                {
                    TR_S(prtstring("DATBuf Fail !\r\n"));
                ...
                SDMMC_SetBusTiming(u8Slot, EV_BUS_HS_SDR25);
        ...
        else //SD3.0
        {
            ...
            TR_S(prtstring("[SET_GRP1] SD3.0 (AS: ")); TR_S(prtU8Hex(_stSDMMCInfo[u8Slot].u8AccessMode));
            TR_S(prtstring(")(BS: ")); TR_S(prtU8Hex(eBusTiming)); TR_S(prtstring(") = "));
            ...
            {
                TR_S(prtstring("RspBuf Fail !\r\n"));
                return EV_OTHER_ERR;
            }

            TR_S(prtstring("Pass\r\n"));

            TR_S(prtstring("[SET_GRP1] Clock=")); TR_S(prtUInt(u32MaxClk));
            TR_S(prtstring("\r\n"));


            _stSDMMCInfo[u8Slot].u32MaxClk = u32MaxClk;
            SDMMC_SetBusTiming(u8Slot, eBusTiming);
 ...
 * @return U16_T  : U16 Error Code
 ----------------------------------------------------------------------------------------------------------*/
U16_T SDMMC_Init(U8_T u8Slot, U8_T u8Config)
{
    IPEmType eIP  = ge_IPSlot[u8Slot];

    _SDMMC_InfoInit(u8Slot, u8Config);

    SDMMC_SwitchPAD(u8Slot);

    SDMMC_SetPADVdd(u8Slot, EV_BUS_3_3V);
...
void SDMMC_SetSDRScanVal(U8_T u8Slot, U8_T u8Skew4, U8_T u8Skew4Inv)
{

    IPEmType eIP = ge_IPSlot[u8Slot];

    Hal_SDMMC_SetSDRSCanVal(eIP, u8Skew4, u8Skew4Inv);
}


void SDMMC_SetDDRScanVal(U8_T u8Slot, U8_T u8DQS_Mode, U8_T u8DQS_DLY_Sel)
{
    IPEmType eIP = ge_IPSlot[u8Slot];

    Hal_SDMMC_SetDDRScanVal(eIP, u8DQS_Mode, u8DQS_DLY_Sel);
...
U16_T SDMMC_ADMA_BLK_R(U8_T u8Slot, U32_T u32CardAddr, U32_T *pu32ArrDAddr, U16_T *pu16ArrBCnt, U16_T u16ItemCnt, volatile void *pDMATable)
{
    ...
    for(u16Item=0; u16Item<u16ItemCnt; u16Item++ )
    {
        ...
        Hal_SDMMC_ADMASetting((volatile void *)pDMATable, u16Item, pu16ArrBCnt[u16Item]<<9, pu16ArrBCnt[u16Item], (U32_T)pu32ArrDAddr[u16Item], 0, bEnd);
    }
...
U16_T SDMMC_ADMA_BLK_W(U8_T u8Slot, U32_T u32CardAddr, U32_T *pu32ArrDAddr, U16_T *pu16ArrBCnt, U16_T u16ItemCnt, volatile void *pDMATable)
{
    ...
    for(u16Item=0; u16Item<u16ItemCnt; u16Item++ )
    {
        ...
        u16BlkCnt +=pu16ArrBCnt[u16Item];
        Hal_SDMMC_ADMASetting((volatile void *)pDMATable, u16Item, pu16ArrBCnt[u16Item]<<9, pu16ArrBCnt[u16Item], pu32ArrDAddr[u16Item], 0, bEnd);
    }
