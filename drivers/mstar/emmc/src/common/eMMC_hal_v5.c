...
void eMMC_FCIE_DumpRegisters(void)
{
	...
	for(u16_i=0 ; u16_i<0x80; u16_i++)
	{
		...
		REG_FCIE_R(GET_REG_ADDR(PAD_TOP0_BASE, u16_i), u16_reg);
		eMMC_debug(eMMC_DEBUG_LEVEL, 0, "%04Xh ", u16_reg);
	}
	...
	for(u16_i=0 ; u16_i<0x80; u16_i++)
	{
		...
		REG_FCIE_R(GET_REG_ADDR(PAD_TOP1_BASE, u16_i), u16_reg);
		eMMC_debug(eMMC_DEBUG_LEVEL, 0, "%04Xh ", u16_reg);
	}
...
U32 eMMC_FCIE_Init(void)
{
	...
	#if 0 //eMMC_TEST_IN_DESIGN
	{
		...
		{
			eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: Reg0x30h BIST fail: %04Xh \r\r\n", u16_reg);
			return eMMC_ST_ERR_BIST_FAIL;
		}
		if(u16_reg & BIT_FCIE_DEBUG_MODE_MASK)
		{
			eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: Reg0x30h Debug Mode: %04Xh \r\r\n", u16_reg);
...
)
{
    ...
    #if 0 //defined(FCIE_ZDEC_TALK_TEST) && FCIE_ZDEC_TALK_TEST
...
U32 eMMC_WaitSetCIFD(U8 * pu8_DataBuf, U32 u32_ByteCnt)
{
	...
	{
		eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "Error: CIFD timeout 0, ErrCode:%Xh\r\r\n", eMMC_ST_ERR_TIMEOUT_WAITCIFDEVENT);
	...
	{	// check status
		...
		if(u16_reg & (BIT_SD_RSP_TIMEOUT|BIT_SD_RSP_CRC_ERR))
		{
			if(u8_retry_fcie < eMMC_CMD_API_ERR_RETRY_CNT && 0==eMMC_IF_TUNING_TTABLE())
			{
				u8_retry_fcie++;
				eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
				    "eMMC WARN: CMD12 check SD_STSh error: %Xh, %Xh, retry: %u\r\n",
				    u16_reg, u32_err, u8_retry_fcie);
				eMMC_FCIE_ErrHandler_ReInit();
				return u32_err;
			}
			...
			eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
				"eMMC Err: CMD12 check SD_STSh error: %Xh, %Xh, retry: %u\r\n",
				u16_reg, u32_err, u8_retry_fcie);
			eMMC_FCIE_ErrHandler_Stop();
		}
		else
