...
int MDrv_SPINAND_WriteCIS_for_ROM(SPINAND_FLASH_INFO_TAG_t *pSpiNandInfoTagOut)
{
...
//        if (ptPartInfo->u16_SpareByteCnt == pSpiNandDrv->tSpinandInfo.u16_SpareByteCnt &&
//            ptPartInfo->u16_PageByteCnt == pSpiNandDrv->tSpinandInfo.u16_PageByteCnt &&
//            ptPartInfo->u16_BlkPageCnt == pSpiNandDrv->tSpinandInfo.u16_BlkPageCnt &&
//            ptPartInfo->u16_BlkCnt == pSpiNandDrv->tSpinandInfo.u16_BlkCnt) //Match Partition layout
        {
...
//        else
//        {
//            spi_nand_msg("PNI match fail");
//            dumpPartition(ptPartInfo ,pSpiNandDrv);
//            return -1;
//        }
    }
...
int MDrv_SPINAND_GetMtdParts(char *buf)
{
	...
	Maxlen -= 15;
	#if 0
	sprintf(tmp, "0x%X(CIS)", 10 * pSpiNandDrv->u16_BlkPageCnt * pSpiNandDrv->u16_PageByteCnt);
	len = strlen(tmp);
	...
	for(u8_i = 0; u8_i < pPartInfo->u16_PartCnt; u8_i ++)
	{
		...
		if (u8_i)
			...
			sprintf(tmp, "0x%X@0x%X(%s)", (unsigned int)PartSize, 10 * pSpiNandDrv->tSpinandInfo.u16_BlkPageCnt * pSpiNandDrv->tSpinandInfo.u16_PageByteCnt, PartName); //,pPartInfo->records[u8_i].u16_PartType);
