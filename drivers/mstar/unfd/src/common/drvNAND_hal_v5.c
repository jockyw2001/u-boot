...
U32 NC_CheckNANDIdleStatus(void)
{
	if(((REG_FCIE(NC_DATA_STS) & 0xFF) != 0xFF))
		return UNFD_ST_ERR_NO_NFIE;
	...
	if(NC_WaitComplete(BIT_NC_JOB_END, DELAY_100ms_in_us) == DELAY_100ms_in_us)
	{
		nand_debug(UNFD_DEBUG_LEVEL_HIGH, 1, "Error: NC_ReadStatus Timeout\r\n");
		NC_CLR_DDR_MODE();
		return UNFD_ST_ERR_R_TIMEOUT;
	}

	if((REG_FCIE(NC_ST_READ) & (BIT7|BIT0)) != BIT7)
		return UNFD_ST_ERR_NO_NFIE;
