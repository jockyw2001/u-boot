...
ifeq ($(CONFIG_MS_DISP),y)
PLATFORM_LIBGCC := -L $(shell dirname `$(CC) $(c_flags) -print-libgcc-file-name`)/arm-linux-gnueabi -lgcc
else
...
ifeq ($(CONFIG_MS_DISP),y)
DISP_LIBS := $(shell find $(srctree)/drivers/mstar/disp/lib/ -name '*.lib')
endif
...
quiet_cmd_u-boot__ ?= LD      $@
      ...
      --start-group $(u-boot-main) $(DISP_LIBS) --end-group                 \
