...
{
    U16 hwstrap, uart_loader;
	hwstrap = (INREG16( GET_REG_ADDR(MS_BASE_REG_CHIPTOP_PA, 0x65) )) & 0x0F;
	uart_loader = (INREG16( GET_REG_ADDR(MS_BASE_REG_CHIPTOP_PA, 0x65) ))& 0x10; /*Default high*/

    if(hwstrap == BIT1)
    {
        return DEVINFO_BOOT_TYPE_SPINOR;
    }
    else if(hwstrap == (BIT2|BIT0))
    {
        return DEVINFO_BOOT_TYPE_P_NAND;
    }
    else if(hwstrap == BIT2)
    {
        return DEVINFO_BOOT_TYPE_EMMC;
    }
    else if((hwstrap == BIT1))
    {
		return DEVINFO_BOOT_TYPE_SPINAND;
    }
	else if((hwstrap == 0x0))
    {
		if (uart_loader != 0)
		return DEVINFO_BOOT_TYPE_USB;
		else
		return DEVINFO_BOOT_TYPE_UART;
    }
    else
    {
...
#define  reg_flush_op_on_fire           (0x1F000000 + 0x101800*2 + 0x45*4) // Write 0x1018_45[0] = 1, flush fire trigger
#define  reg_Flush_miu_pipe_done_flag   (0x1F000000 + 0x101800*2 + 0x50*4) // Read 0x1018_50[12] = 1, flush all finish
...
inline void _chip_flush_miu_pipe(void)
{
	...
	*(volatile unsigned short *)(reg_flush_op_on_fire) &= ~(0x0001);
	*(volatile unsigned short *)(reg_flush_op_on_fire) |= 0x0001;
