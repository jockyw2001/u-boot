...
void do_io_ddr_test(unsigned char u8Slot)
{
    unsigned char u8DQS_Mode =0 , u8DQS_DLY_Sel = 0;

    for(u8DQS_Mode=1;u8DQS_Mode<=7;u8DQS_Mode++)
    {

        //for(u8DQS_DLY_Sel=0;u8DQS_DLY_Sel<=15;u8DQS_DLY_Sel++)
        {
            IPV_SDMMC_SetDDRScanVal(u8Slot, u8DQS_Mode, u8DQS_DLY_Sel);

            IPV_SDMMC_DMA_RW(u8Slot, 0, 1, (FALSE));
...
int do_sdmmc_test ( cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    case 4:
        ...
        if (strcmp(argv[1], "init") == 0)
            IPV_SDMMC_Init(u8Val1, (unsigned char)u32Val1);
...
//###########################################################################################################
        ...
        else if (strcmp(argv[1], "at30") == 0)
        {
            IPV_SDMMC_TimerTest(3);
            IPV_SDMMC_CardDetect(u8Val1);

            printf("\r\n$$$$$$$$$$$$$$$$$$$$ [ Test SD Spec 3.0  ] $$$$$$$$$$$$$$$$$$$\r\n");
            IPV_SDMMC_Init(u8Val1, 1);
...
U_BOOT_CMD(
    ...
    "Mstar SD 2.0/3.0 Ver33 IP Verification System",
    "[OPTION] <parameter>\n"
    "===============================================================================\n"
