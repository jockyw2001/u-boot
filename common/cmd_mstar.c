...
#if defined(CONFIG_MS_SDMMC) || defined(CONFIG_MS_USB) || defined(CONFIG_CMD_NET)
...
extern int MDrv_SPINAND_WriteCIS_for_ROM(SPINAND_FLASH_INFO_TAG_t *pSpiNandInfoTagOut);
...
int writeSpinandCIS(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	spi_nand_err("Device detect! MID =%x, DID =%x",pSpiNandDrv->tSpinandInfo.au8_ID[0], pSpiNandDrv->tSpinandInfo.au8_ID[1]);

    u32_CISAddr = simple_strtoul(argv[1], NULL, 0);
    u32_partInfo = simple_strtoul(argv[2], NULL, 0);

    if(MDrv_SPINAND_SearchCIS_in_DRAM((u8 *)u32_CISAddr, (u8 *)u32_partInfo, &pSpiNandInfoTagOut) != SPINAND_SUCCESS)
    {
...
extern  int eMMC_CMD8_MIU(unsigned char pu8_DataBuf[512]);
...
int do_wdt_enable (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	u32Timer=simple_strtoul(argv[1], NULL, 0);
	setenv_ulong(WDT_TIMEOUT_ENV_STR, u32Timer);
	hw_watchdog_init();
    return 0;
}

U_BOOT_CMD(
	wdt_enable, CONFIG_SYS_MAXARGS, 1,    do_wdt_enable,
	"wdt_enable n - set Watchdog timer to n secs \n",
	NULL
);
void hw_watchdog_disable(void);

int do_wdt_disable (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	hw_watchdog_disable();
    return 0;
}
...
U_BOOT_CMD(
	wdt_disable, CONFIG_SYS_MAXARGS, 1,    do_wdt_disable,
	"wdt_disable - disable Watchdog \n",
	NULL
);

#endif
#if defined(CONFIG_MS_PWM)
int pwmtest(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    div = simple_strtoul(argv[2], NULL, 0);
    invert = simple_strtoul(argv[3], NULL, 0);
    duty = simple_strtoul(argv[4], NULL, 0);
    period_ns = simple_strtoul(argv[5], NULL, 0);

	pwm_init(id, div, invert);
	pwm_config(id, duty, period_ns);
    return 0;
}

U_BOOT_CMD(
    pwm,  CONFIG_SYS_MAXARGS,    1,    pwmtest,
    "id:0-7 div:0-65535 invert:0-1 duty:0-100 period:2~262143",
    "FREQ:12M"
    ""
);
#endif
#if defined(CONFIG_MS_DISP)
extern  void Disp_Verify(int argc, char * const argv[]);
int do_display (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	Disp_Verify(argc, argv);
    return 0;
}
...
#if defined(CONFIG_MS_SATA_HOST)
extern void sata_demo(void);
int do_sata (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    printf("\nsata demo test program.....\n\n");
...
U_BOOT_CMD(
	sata, CONFIG_SYS_MAXARGS, 1,    do_sata,
	"run sata test program\n",
	NULL
);
#endif
